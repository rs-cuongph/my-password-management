// Load UMD build and get from global
declare global {
  interface Window {
    argon2: any;
  }
...
const loadArgon2UMD = () => {
  return new Promise((resolve, reject) => {
    ...
    script.src = '/node_modules/argon2-browser/dist/argon2-bundled.min.js';
    script.onload = () => {
      if (window.argon2) {
        resolve(window.argon2);
      } else {
        reject(new Error('argon2 not found on window'));
      }
    };
...
export async function deriveMasterKey(
  ...
  try {
    ...
    if (!serverSalt || serverSalt.length === 0) {
      throw new Error('Không tìm thấy salt từ server. Vui lòng đăng nhập lại.');
    }
    ...
    const saltBytes = new Uint8Array(
      serverSalt.match(/.{1,2}/g)!.map((byte) => parseInt(byte, 16))
    );
    ...
    const argon2: any = await loadArgon2UMD();
    ...
    if (error instanceof Error) {
      if (error.message.includes('memory')) {
        throw new Error(
          'Insufficient memory for key derivation. Please close other applications and try again.'
        );
      }
      ...
      if (error.message.includes('salt')) {
        throw new Error('Salt không hợp lệ từ server. Vui lòng đăng nhập lại.');
      }
    }
...
export async function verifyMasterPassword(
  ...
  try {
    ...
    if (!kdfParams.salt || kdfParams.salt.length === 0) {
      throw new Error(
        'Không tìm thấy salt trong tham số KDF. Vui lòng đăng nhập lại.'
      );
    }
    ...
    // Convert hex salt to Uint8Array
    const saltBytes = new Uint8Array(
      kdfParams.salt.match(/.{1,2}/g)!.map((byte) => parseInt(byte, 16))
    );
    ...
    if (error instanceof Error) {
      if (error.message.includes('memory')) {
        throw new Error(
          'Insufficient memory for key verification. Please close other applications and try again.'
        );
      }
      if (error.message.includes('timeout')) {
      ...
      if (error.message.includes('salt')) {
        throw new Error(
          'Salt không hợp lệ trong tham số đã lưu. Vui lòng đăng nhập lại.'
        );
      }
    }
 ...
 */
export function validateArgon2Params(params: Argon2Params): {
  valid: boolean;
  error?: string;
} {
  if (params.memory < 64 * 1024 || params.memory > 128 * 1024) {
  ...
  if (params.hashLength < 16 || params.hashLength > 64) {
    return {
      valid: false,
      error: 'Hash length must be between 16 and 64 bytes',
    };
  }
