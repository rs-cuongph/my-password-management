...
export const srOnlyClass = 'absolute -m-px w-px h-px p-0 border-0 overflow-hidden whitespace-nowrap clip-rect-0';
...
export const useRovingTabindex = (
  ...
  useEffect(() => {
    ...
    const items = containerRef.current.querySelectorAll('[role="tab"], [role="menuitem"], [role="option"]');
  ...
  const handleKeyDown = useCallback((event: React.KeyboardEvent, currentIndex: number, onIndexChange: (index: number) => void) => {
    if (!containerRef.current) return;
